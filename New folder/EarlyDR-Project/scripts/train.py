...
def gather_samples(data_dir):
    ...
    for label, folder in enumerate(FOLDER_NAMES):
        ...
        if not os.path.isdir(folder_path):
            continue
        fnames = sorted(os.listdir(folder_path))  # sorted: os.listdir() order is NOT
        for fname in fnames:                       # guaranteed identical across OS/filesystems --
            if fname.lower().endswith((".png", ".jpg", ".jpeg")):  # unsorted input would silently
                samples.append((os.path.join(folder_path, fname), label))  # desync the train/val/test
    return samples                                  # split between team members using the same seed.
